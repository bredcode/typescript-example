var gatling = (() => {
  ...
  __export(stdin_exports, {
    lotto: () => lotto_gatling_default
  });

  // src/lotto.gatling.js
  var import_core = __toESM(require_target());
  var import_http = __toESM(require_target2());
  var httpProtocol = import_http.http.baseUrl("http://localhost:5000").acceptHeader("application/json");
  var lottoScenario = (0, import_core.scenario)("Lotto Load Test").exec(
    (0, import_http.http)("Lotto API Request").get("/lotto").check((0, import_http.status)().is(200)),
    // 응답 코드 200 확인
